...
/** Presenter associated to {@link StarredmessagesFragment} */
public class StarredmessagesPresenter implements MainContracts.messageListPresenter {
     ...
     * {@link StarredmessagesPresenter#onLoadMessagesTaskComplete(QuerySnapshot)} has to be executed */
    private QuerySnapshot results;

    /** This flag is set when {@link StarredmessagesPresenter#onUpdateTaskComplete()}
    ...
    public void loadmessages() {
        ...
        reader.readStarredmessages(FirebaseAuthHelper.getUserId());
    }
        ...
        if (results != null) {
            for (QueryDocumentSnapshot snap: results) {
                ...
                message message = new message(id, userId, userName, title, description, reply,
                        timestamp, (int) nStars, position, status, true);
